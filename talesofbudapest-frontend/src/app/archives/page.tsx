  ...
  return (
    <main className="flex h-[100dvh] flex-col bg-background">
      <div className="flex-1 overflow-y-auto px-4 pb-24 pt-[max(1.5rem,env(safe-area-inset-top))]">
        <p className="text-label text-accent">Archives</p>
        <h1 className="text-headline mt-2 text-on-surface">Saved narratives</h1>
        <p className="mt-2 text-body text-on-surface/55">
          Your custom walking stories, ready to revisit.
        </p>

        {isLoading && (
          <div className="mt-10">
            <LoadingScreen message="Loading archives…" />
          </div>
        )}
        ...
        {!isLoading && !error && narratives.length === 0 && (
          <p className="mt-10 text-body text-on-surface/45">
            No narratives yet. Tap the prompt bar on the map to weave your first story.
          </p>
        )}
        ...
        <ul className="mt-8 flex flex-col gap-3">
          {narratives.map((narrative) => (
            <li key={narrative.id}>
              ...
              >
                <p className="text-[1.0625rem] font-semibold tracking-tight text-on-surface">{narrative.title}</p>
                ...
                <div className="mt-3 flex items-center gap-3 text-[0.6875rem] uppercase tracking-[0.12em] text-on-surface/40">
                  <span>{narrative.chapterCount} chapters</span>
                  <span>•</span>
