...
import { PROMPT_BAR_PLACEHOLDER } from '@/constants/narrative'
...
export const PromptBar = ({ onOpen, onMicClick }: PromptBarProps) => (
  <div className="prompt-bar-glass flex h-12 w-full items-center gap-3 rounded-full px-4">
    <button
      type="button"
      onClick={onOpen}
      aria-label={PROMPT_BAR_PLACEHOLDER}
      className="flex min-w-0 flex-1 items-center gap-3 text-left"
    >
      <Sparkles className="h-5 w-5 shrink-0 text-accent" strokeWidth={2} aria-hidden="true" />
      <span className="truncate text-body font-medium text-on-surface/60">{PROMPT_BAR_PLACEHOLDER}</span>
    </button>
    <button
      type="button"
      onClick={() => {
