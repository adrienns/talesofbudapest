-- Durable rate limits and idempotency for public AI/TTS endpoints.

alter table public.narratives
  add column if not exists idempotency_key text;

create unique index if not exists narratives_owner_idempotency_idx
  on public.narratives (owner_id, idempotency_key)
  where owner_id is not null and idempotency_key is not null;

create table if not exists public.expensive_request_events (
...
create index if not exists expensive_request_events_lookup_idx
  on public.expensive_request_events (actor_key, action, created_at desc);

alter table public.expensive_request_events enable row level security;
...
revoke all on table public.expensive_request_events from public, anon, authenticated;
