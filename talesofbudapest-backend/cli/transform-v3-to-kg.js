...
const main = async () => {
  ...
  // Latest complete non-experiment record per page set wins.
  const latest = new Map();
  for (const row of itemRows) {
    if (row.experiment_id || row.status === 'preflight' || !Array.isArray(row.items)) continue;
  ...
  for (const [key, run] of latest) {
    ...
    for (const entity of run.entity_aliases ?? []) {
